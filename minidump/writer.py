...
from minidump.utils.winapi.kernel32 import (
    OpenProcess,
    PROCESS_ALL_ACCESS,
    VirtualQueryEx,
    ReadProcessMemory,
)
from minidump.utils.winapi.psapi import (
    EnumProcessModules,
...
class LiveSystemReader(MinidumpSystemReader):
    ...
    def get_sysinfo(self):
        ...
        sysinfo.ProcessorArchitecture = PROCESSOR_ARCHITECTURE(
            sysinfo_raw.id.w.wProcessorArchitecture
        )
    ...
    def get_modules(self):
        ...
        for module in EnumProcessModules(self.process_handle):
            ...
            mmod.TimeDateStamp = (
                fileversion.dwFileDateMS << 32 + fileversion.dwFileDateLS
            )
    ...
    def get_memory(self):
        ...
        for section in self.meminfolist.entries:
            if section.Protect in read_flags:
                ...
                data = ReadProcessMemory(
                    self.process_handle, section.BaseAddress, section.RegionSize
                )
                input(data)
...
class MinidumpWriter:
    ...
    def prepare_header(self):
        ...
        self.header.NumberOfStreams = (
            len(self.streams) + 1
        )  # +1 is fot he memory info stream
    ...
    def finalize_header(self):
        ...
        header_size += (
            len(self.streams) * 8
        )  # this is for the dictionary itself, not the streams
