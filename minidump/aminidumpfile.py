    ...
    async def __parse_header(self):
        ...
        for i in range(0, self.header.NumberOfStreams):
            ...
            if minidump_dir:
                ...
                user_stream_type_value = int.from_bytes(
                    t, byteorder="little", signed=False
                )
                logging.debug(
                    "Found Unknown UserStream directory Type: %x"
                    % (user_stream_type_value)
                )
    ...
    async def __parse_directories(self):
        ...
        for dir in self.directories:
            if dir.StreamType == MINIDUMP_STREAM_TYPE.UnusedStream:
                ...
                logging.debug(
                    "Found ModuleListStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                ...
                logging.debug(
                    "Found MemoryListStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                self.memory_segments = await MinidumpMemoryList.aparse(
                    dir, self.file_handle
                )
                # logging.debug(str(self.memory_segments))
                continue
            elif dir.StreamType == MINIDUMP_STREAM_TYPE.SystemInfoStream:
                logging.debug(
                    "Found SystemInfoStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                ...
                logging.debug(
                    "Found ThreadExListStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                self.threads_ex = await MinidumpThreadExList.aparse(
                    dir, self.file_handle
                )
                ...
                logging.debug(
                    "Found Memory64ListStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                self.memory_segments_64 = await MinidumpMemory64List.aparse(
                    dir, self.file_handle
                )
                ...
                logging.debug(
                    "Found ExceptionStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                ...
                logging.debug(
                    "Found HandleDataStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                self.handles = await MinidumpHandleDataStream.aparse(
                    dir, self.file_handle
                )
                ...
                logging.debug(
                    "Found FunctionTableStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                ...
                logging.debug(
                    "Found UnloadedModuleListStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                self.unloaded_modules = await MinidumpUnloadedModuleList.aparse(
                    dir, self.file_handle
                )
                ...
                logging.debug(
                    "Found MemoryInfoListStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                self.memory_info = await MinidumpMemoryInfoList.aparse(
                    dir, self.file_handle
                )
                ...
                logging.debug(
                    "Found ThreadInfoListStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                self.thread_info = await MinidumpThreadInfoList.aparse(
                    dir, self.file_handle
                )
                ...
                logging.debug(
                    "Found SystemMemoryInfoStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                logging.debug(
                    "SystemMemoryInfoStream parsing is not implemented (Missing documentation)"
                )
                continue

            elif dir.StreamType == MINIDUMP_STREAM_TYPE.JavaScriptDataStream:
                logging.debug(
                    "Found JavaScriptDataStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                logging.debug(
                    "JavaScriptDataStream parsing is not implemented (Missing documentation)"
                )

            elif dir.StreamType == MINIDUMP_STREAM_TYPE.ProcessVmCountersStream:
                logging.debug(
                    "Found ProcessVmCountersStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                logging.debug(
                    "ProcessVmCountersStream parsing is not implemented (Missing documentation)"
                )

            elif dir.StreamType == MINIDUMP_STREAM_TYPE.TokenStream:
                logging.debug(
                    "Found TokenStream @%x Size: %d"
                    % (dir.Location.Rva, dir.Location.DataSize)
                )
                logging.debug(
                    "TokenStream parsing is not implemented (Missing documentation)"
                )

            else:
                logging.debug(
                    "Found Unknown Stream! Type: %s @%x Size: %d"
                    % (dir.StreamType.name, dir.Location.Rva, dir.Location.DataSize)
                )
