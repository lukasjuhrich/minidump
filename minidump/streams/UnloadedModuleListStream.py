...
class MINIDUMP_UNLOADED_MODULE_LIST:
    ...
    def parse(buff):
        ...
        muml.SizeOfHeader = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        muml.SizeOfEntry = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        muml.NumberOfEntries = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        return muml
...
class MINIDUMP_UNLOADED_MODULE:
    ...
    def parse(buff):
        ...
        mum.TimeDateStamp = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mum.ModuleNameRva = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        return mum
