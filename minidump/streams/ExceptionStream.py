...
class MINIDUMP_EXCEPTION_STREAM:
    ...
    @staticmethod
    def get_header():
        return ["ThreadId", *MINIDUMP_EXCEPTION.get_header()]

    def to_row(self):
        return ["0x%08x" % self.ThreadId, *self.ExceptionRecord.to_row()]
...
class ExceptionCode(enum.Enum):
    ...
    EXCEPTION_SIGPIPE = (
        0x0000000D  # Write on a pipe with no reader, Broken pipe (POSIX)
    )
    ...
    EXCEPTION_SIGCHLD = (
        0x00000011  # Child process has stopped or exited, changed (POSIX)
    )
    ...
    EXCEPTION_SIGTTIN = (
        0x00000015  # Background process trying to read, from TTY (POSIX)
    )
...
class MINIDUMP_EXCEPTION:
    ...
    def parse(buff):
        ...
        me.ExceptionCode_raw = int.from_bytes(
        ...
        me.ExceptionFlags = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        me.ExceptionRecord = int.from_bytes(
            buff.read(8), byteorder="little", signed=False
        )
        me.ExceptionAddress = int.from_bytes(
            buff.read(8), byteorder="little", signed=False
        )
        me.NumberParameters = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        me.__unusedAlignment = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
    ...
    def __str__(self):
        ...
        t += "ExceptionInformation : %s\n" % ";".join(
            "0x%x" % info for info in self.ExceptionInformation
        )
        return t
