...
class PRODUCT_TYPE(enum.Enum):
    VER_UNIDENTIFIED_PRODUCT = (
        0x0000000  # Crashpad des not set ProductType value on non-Windows systems
    )
...
class MINIDUMP_SYSTEM_INFO:
    ...
    def to_bytes(self, data_buffer=None):
        t = self.ProcessorArchitecture.value.to_bytes(
        ...
        if self.ProcessorArchitecture == PROCESSOR_ARCHITECTURE.INTEL:
            ...
            t += self.VersionInformation.value.to_bytes(
                4, byteorder="little", signed=False
            )
            t += self.FeatureInformation.value.to_bytes(
                4, byteorder="little", signed=False
            )
            t += self.AMDExtendedCpuFeatures.value.to_bytes(
                4, byteorder="little", signed=False
            )
        else:
    ...
    def parse(buff):
        ...
        msi.ProcessorArchitecture = PROCESSOR_ARCHITECTURE(
            int.from_bytes(buff.read(2), byteorder="little", signed=False)
        )
        msi.ProcessorLevel = int.from_bytes(
            buff.read(2), byteorder="little", signed=False
        )
        msi.ProcessorRevision = int.from_bytes(
            buff.read(2), byteorder="little", signed=False
        )
        ...
        if msi.ProcessorArchitecture == PROCESSOR_ARCHITECTURE.INTEL:
            ...
            msi.VersionInformation = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
            msi.FeatureInformation = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
            msi.AMDExtendedCpuFeatures = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
        else:
            for _ in range(2):
                msi.ProcessorFeatures.append(
...
class MinidumpSystemInfo:
    ...
    def guess_os(self):
        if (
            ...
            and self.MinorVersion == 0
            and self.ProductType == PRODUCT_TYPE.VER_NT_WORKSTATION
        ):
            self.OperatingSystem = "Windows 10"
        elif (
            self.MajorVersion == 10
            and self.MinorVersion == 0
            and self.ProductType != self.ProductType.VER_NT_WORKSTATION
        ):
            self.OperatingSystem = "Windows Server 2016 Technical Preview"
        elif (
            self.MajorVersion == 6
            and self.MinorVersion == 3
            and self.ProductType == self.ProductType.VER_NT_WORKSTATION
        ):
    ...
    def parse(dir, buff):
        ...
        try:
            ...
            logging.log(
                1,
                "Failed to guess OS! MajorVersion: %s MinorVersion %s BuildNumber %s ProductType: %s"
                % (t.MajorVersion, t.MinorVersion, t.BuildNumber, t.ProductType),
            )
    ...
    def __str__(self):
        ...
        t += "ProcessorFeatures %s\n" % " ".join(
            [hex(x) for x in self.ProcessorFeatures]
        )
