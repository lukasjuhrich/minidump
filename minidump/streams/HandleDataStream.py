...
class MINIDUMP_HANDLE_DATA_STREAM:
    ...
    def parse(buff):
        ...
        mhds.SizeOfHeader = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mhds.SizeOfDescriptor = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mhds.NumberOfDescriptors = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
...
class MINIDUMP_HANDLE_DESCRIPTOR_2:
    ...
    def parse(buff):
        ...
        mhd.ObjectInfoRva = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
...
class MINIDUMP_HANDLE_OBJECT_INFORMATION:
    ...
    def parse(buff):
        ...
        mhoi.NextInfoRva = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
...
class MinidumpHandleDescriptor:
    ...
    async def aparse(t, buff):
        ...
        if isinstance(t, MINIDUMP_HANDLE_DESCRIPTOR_2):
            if t.ObjectInfoRva is not None and t.ObjectInfoRva != 0:
                await MinidumpHandleDescriptor.awalk_objectinfo(
                    mhd, t.ObjectInfoRva, buff
                )
        return mhd
