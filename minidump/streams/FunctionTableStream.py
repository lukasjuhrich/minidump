...
class MINIDUMP_FUNCTION_TABLE_STREAM:
    ...
    def parse(dir, buff):
        ...
        mfts.SizeOfHeader = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mfts.SizeOfDescriptor = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mfts.SizeOfNativeDescriptor = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mfts.SizeOfFunctionEntry = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mfts.NumberOfDescriptors = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mfts.SizeOfAlignPad = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
