...
class MINIDUMP_MISC_INFO_2:
    ...
    def parse(buff):
        ...
        mmi.SizeOfInfo = int.from_bytes(buff.read(4), byteorder="little", signed=False)
        mmi.Flags1 = MinidumpMiscInfo2Flags1(
            int.from_bytes(buff.read(4), byteorder="little", signed=False)
        )
        ...
        if mmi.Flags1 & MinidumpMiscInfo2Flags1.MINIDUMP_MISC1_PROCESSOR_POWER_INFO:
            mmi.ProcessorMaxMhz = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
            mmi.ProcessorCurrentMhz = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
            mmi.ProcessorMhzLimit = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
            mmi.ProcessorMaxIdleState = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
            mmi.ProcessorCurrentIdleState = int.from_bytes(
                buff.read(4), byteorder="little", signed=False
            )
        else:
            buff.read(20)
