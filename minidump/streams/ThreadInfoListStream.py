...
class DumpFlags(enum.Enum):
    ...
    MINIDUMP_THREAD_INFO_INVALID_CONTEXT = (
        0x00000010  # Thread context could not be retrieved.
    )
    MINIDUMP_THREAD_INFO_INVALID_INFO = (
        0x00000008  # Thread information could not be retrieved.
    )
    MINIDUMP_THREAD_INFO_INVALID_TEB = (
        0x00000020  # TEB information could not be retrieved.
    )
    MINIDUMP_THREAD_INFO_WRITING_THREAD = (
        0x00000002  # This is the thread that called MiniDumpWriteDump.
    )
...
class MINIDUMP_THREAD_INFO_LIST:
    ...
    def parse(buff):
        ...
        mtil.SizeOfHeader = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mtil.SizeOfEntry = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mtil.NumberOfEntries = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
...
class MINIDUMP_THREAD_INFO:
    ...
    def parse(buff):
        ...
        mti.StartAddress = int.from_bytes(
...
class MinidumpThreadInfo:
    ...
    def __str__(self):
        return (
            "ThreadId: %x DumpFlags: %s DumpError: %s ExitStatus: %x CreateTime: %s ExitTime: %s KernelTime: %s UserTime: %s StartAddress: %x Affinity: %d"
            % (
                self.ThreadId,
                self.DumpFlags,
                self.DumpError,
