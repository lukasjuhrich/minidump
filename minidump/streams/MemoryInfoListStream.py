...
class MINIDUMP_MEMORY_INFO_LIST:
    ...
    def parse(buff):
        ...
        mhds.SizeOfEntry = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mhds.NumberOfEntries = int.from_bytes(
...
class MINIDUMP_MEMORY_INFO:
    ...
    def parse(buff):
        ...
        mmi.AllocationBase = int.from_bytes(
            buff.read(8), byteorder="little", signed=False
        )
        mmi.AllocationProtect = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mmi.__alignment1 = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
        mmi.RegionSize = int.from_bytes(buff.read(8), byteorder="little", signed=False)
        try:
            mmi.State = MemoryState(
                int.from_bytes(buff.read(4), byteorder="little", signed=False)
            )
        except:
            pass
        try:
            mmi.Protect = AllocationProtect(
                int.from_bytes(buff.read(4), byteorder="little", signed=False)
            )
        except:
            pass
        try:
            ...
            pass
        mmi.__alignment2 = int.from_bytes(
            buff.read(4), byteorder="little", signed=False
        )
