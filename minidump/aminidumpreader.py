...
class AMinidumpBufferedMemorySegment:
    def __init__(self, memory_segment, chunksize=10 * 1024):
        ...
        self.total_size = (
            memory_segment.end_virtual_address - memory_segment.start_virtual_address
        )
    ...
    async def read(self, file_handle, start, end):
        if end is None:
            ...
            return await file_handle.read(
                self.end_address - (self.start_file_address + start)
            )
...
class AMinidumpBufferedReader:
    ...
    async def _select_segment(self, requested_position):
        ...
        for memory_segment in self.reader.memory_segments:
            if memory_segment.inrange(requested_position):
                newsegment = AMinidumpBufferedMemorySegment(
                    memory_segment, chunksize=self.segment_chunk_size
                )
        ...
        raise Exception(
            "Memory address 0x%08x is not in process memory space" % requested_position
        )
    ...
    async def align(self, alignment=None):
        ...
        if alignment is None:
            if (
                self.reader.sysinfo.ProcessorArchitecture
    ...
    async def peek(self, length):
        ...
        return await self.current_segment.read(
            self.reader.file_handle,
            self.current_position - self.current_segment.start_address,
            t - self.current_segment.start_address,
        )
    ...
    async def read(self, size=-1):
        ...
        if size == -1:
            ...
            return await self.current_segment.read(
                self.reader.file_handle,
                old_new_pos - self.current_segment.start_address,
                None,
            )
    ...
    async def find_all(self, pattern):
        ...
        last_found = -1
        while True:
            last_found = await self.current_segment.find(
                self.reader.file_handle, pattern, last_found + 1
            )
            if last_found == -1:
                break
    ...
    async def find_in_module(
        self, module_name, pattern, find_first=False, reverse_order=False
    ):
        t = await self.reader.search_module(
            module_name,
            pattern,
...
class AMinidumpFileReader:
    def __init__(self, minidumpfile):
        ...
        self.sizeof_long = 4
        self.unpack_long = "<L"
        if minidumpfile.sysinfo.ProcessorArchitecture in [
            PROCESSOR_ARCHITECTURE.AMD64,
            PROCESSOR_ARCHITECTURE.AARCH64,
        ]:
            self.sizeof_ptr = 8
            self.unpack_ptr = "<Q"
        elif self.sysinfo.ProcessorArchitecture in [
            ...
            raise Exception(
                "Unknown processor architecture %s! Please fix and submit PR!"
                % self.sysinfo.ProcessorArchitecture
            )
    ...
    async def search(self, pattern, find_first=False, chunksize=10 * 1024):
        ...
        for ms in self.memory_segments:
            t += await ms.asearch(
                pattern, self.file_handle, find_first=find_first, chunksize=chunksize
            )
