...
def enum_process_names():
    ...
    for pid in enum_pids():
        ...
        if process_handle is None:
            logging.debug(
                "[Enum Processes]Failed to open process PID: %d Reason: %s "
                % (pid, WinError(get_last_error()))
            )
            continue
        ...
        res = QueryFullProcessImageName(
            process_handle, 0, image_name, ctypes.byref(max_path)
        )
        if res == 0:
            logging.debug(
                "[Enum Proceses]Failed GetProcessImageFileName on PID: %d Reason: %s "
                % (pid, WinError(get_last_error()))
            )
            continue
...
def create_dump(pid, output_filename, mindumptype, with_debug=False):
    ...
    if is64 != IS_PYTHON_64:
        logging.warning(
            "process architecture mismatch! This could case error! Python arch: %s Target process arch: %s"
            % ("x86" if not IS_PYTHON_64 else "x64", "x86" if not is64 else "x64")
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Tool to create process dumps using windows API"
    )
    parser.add_argument(
        "-d",
        "--with-debug",
        action="store_true",
        help="enable SeDebugPrivilege, use this if target process is not in the same user context as your script",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
        default=0,
        help="verbosity, add more - see more",
    )
    ...
    dump_group.add_argument(
        "-o", "--outfile", help="Output .dmp file name", required=True
    )
        ...
        if args.name:
            ...
            for pid in pid_to_name:
                if pid_to_name[pid].find(args.name) != -1:
                    ...
                    create_dump(
                        pid, args.outfile, mindumptype, with_debug=args.with_debug
                    )
                    return
